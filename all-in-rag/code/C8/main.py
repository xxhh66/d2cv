...
class RecipeRAGSystem:
    ...
    def _check_api_key(self):
        ...
        检查使用API密钥配置
        """
        use_local_llm = getattr(self.config,'use_local_llm',False)
        if not use_local_llm:
            api_key = os.getenv("MOONSHOT_API_KEY")
            if not api_key:
                logger.warning("未设置MOONSHOT_API_KEY环境变量,请尝试使用本地LLM")
                # self.config.use_local_llm = True
                logger.info("已切换本地LLM模式")
            else:
                logger.info("检查到本地MOONSHOT_API_KEY配置，使用在线LLM")
        else:
            logger.info("使用本地LLM模式，跳过API密钥检查")
